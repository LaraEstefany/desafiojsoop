/*
4) A Lonely Class for Lonely Numbers
...
2 3 has distance 1 from 2has distance 1 from 3
...
class Lonely {
  ...
  listOfAllNum() {
    ...
    for (let i = this.firstNum; i <= this.lastNum; i++) {
      listaNumeros.push(i);
    }
    return listaNumeros;
  };
  ...
  listOfAllPrimes() {
    // pesquisar equação numeros primos
    const primes = [];

    for (let i = 2; i < this.lastNum + 100; i++) {

      let quantidadeDeDivisores = [];

      let contador = 1;
      while (i >= contador) {
        if (i % contador == 0) quantidadeDeDivisores.push(contador);
        contador++;
      }

      if (quantidadeDeDivisores.length <= 2) primes.push(i);
    }
  ...
  montaObjetoInicial() {
    const primes = [...this.listOfAllPrimes()];
    const objectsPrime = [];

    this.listOfAllNum().forEach(num => {

      const minimo = [...primes].reverse().find((element) => element < num);

      const maximo = primes.find((element) => element > num);
  ...
  montaObjetoFinal() {
    ...
    this.montaObjetoInicial().forEach(object => {
      const distancia1 = object.numero - object.primo1;
      const distancia2 = object.primo2 - object.numero;
      let resultado;
      let closestPrime;

      if (distancia1 != isNaN(distancia1) || distancia2 <= distancia1) { resultado = distancia2; closestPrime = object.primo2 };
      if (distancia1 < distancia2) { resultado = distancia1; closestPrime = object.primo1 };
  ...
  record(firstNum, lastNum) {
    this.firstNum = firstNum;
    this.lastNum = lastNum;

    let recordDistancia = 0;
    let frase;
    this.montaObjetoFinal().forEach(objeto => {
      if (objeto.distancia > recordDistancia) {
        recordDistancia = objeto.distancia;
        frase = `number: ${objeto.num}, distance: ${objeto.distancia}, closest: ${objeto.closestPrime}`
      };
    });
    return frase;
  }
...
//perguntar pro Renan sobre o limite de números primos